...
The golden path (`tests/test_end_to_end_golden_path.py::test_golden_path_full_research_lifecycle`) executes the following sequential steps:
...
### Targeted End-to-End Golden Path Test
```bash
cd backend
uv run pytest tests/test_end_to_end_golden_path.py -v
```
...
1. **Zero Fake Metrics**: All evaluations calculate real Intersection-over-Union (IoU) overlaps and confidence thresholds from actual model predictions.
2. **Zero Data Leakage**: Evaluation runs only evaluate against isolated test partitions materialized during dataset preparation.
3. **Strict Lineage Traceability**: Every workflow report links directly back to unique `training_run_id`, `dataset_version`, `eval_id`, and `model_version`.
4. **State Machine Integrity**: Workflows cannot transition to invalid stages; review gates enforce explicit researcher sign-off before expensive operations.
