...
class RegionOfInterest(BaseModel):
    ...
    name: str = Field(
        description="Human-readable region name (e.g. 'Loading Zone A', 'Restricted Corridor')"
    )
    shape_type: RegionShape = Field(
        default=RegionShape.RECTANGLE, description="Shape geometry format"
    )
...
class EventRuleConfig(BaseModel):
    ...
    stopped_speed_threshold_px_s: float = Field(
        ...
        ge=1.0,
        le=100.0,
        description="Speed threshold below which object is considered STOPPED (px/s)",
    )
    proximity_threshold_px: float = Field(
        default=100.0,
        ge=10.0,
        le=500.0,
        description="Proximity distance threshold between objects (px)",
    )
    separation_threshold_px: float = Field(
        default=180.0,
        ge=20.0,
        le=800.0,
        description="Separation distance threshold between objects (px)",
    )
...
class EventEvidence(BaseModel):
    ...
    trigger_rule: str = Field(
        default="", description="Deterministic rule or condition that triggered event"
    )
...
class TemporalEvent(BaseModel):
    ...
    reliability: EventReliability = Field(
        default=EventReliability.HIGH, description="Event derivation reliability"
    )
    event_params: dict[str, Any] = Field(
        default_factory=dict,
        description="Structured parameters (region_id, distance_px, speed_px_s, etc.)",
    )
