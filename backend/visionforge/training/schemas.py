...
class TrainingConfig(BaseModel):
    ...
    preparation_id: str = Field(description="Target dataset preparation transaction ID ('prep_...')")
    ...
    optimizer: str = Field(default="auto", description="Optimizer choice ('SGD', 'Adam', 'AdamW', 'auto')")
    device: str = Field(default="cpu", description="Execution device ('cpu', 'mps', 'cuda', 'colab_gpu')")
    random_seed: int = Field(default=42, ge=0, description="Random seed for 100% reproducibility")
    checkpoint_frequency: int = Field(default=5, ge=1, description="Checkpoint saving frequency (epochs)")
    experiment_name: str = Field(default="yolo11_experiment", description="Human readable experiment name")
...
class MetricSnapshot(BaseModel):
    ...
    map50_95: float = Field(default=0.0, description="Mean Average Precision at IoU=0.50:0.95 (mAP@50:95)")


class EvaluationResult(BaseModel):
    """Separate test set evaluation metrics."""

    eval_timestamp: str = Field(default_factory=lambda: datetime.now(UTC).isoformat(), description="ISO timestamp")
...
class BoundingBox(BaseModel):
    ...
    bbox: list[float] = Field(description="[x_center, y_center, width, height] normalized to [0, 1]")
...
class SmokeTestResult(BaseModel):
    ...
    predictions: list[InferencePrediction] = Field(default_factory=list, description="Test image predictions")
