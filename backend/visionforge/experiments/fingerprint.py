...
def create_dataset_fingerprint(
    ...
    num_classes = len(manifest_data.get("class_names", [])) or int(
        manifest_data.get("num_classes", 0)
    )

    # 2. Combined Master Fingerprint Hash
    combined_payload = (
        f"{dataset_id}:{version}:{preparation_id}:{num_samples}:{num_classes}:{manifest_hash}"
    )
